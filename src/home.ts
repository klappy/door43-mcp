...
<section class="g"><h2>Journeys</h2>${s.journeys.map((j) => `<div class="j"><p><b>${esc(j.recipe)}</b> — ${md(j.about)} <span class="not">args: <code>${esc(j.args)}</code></span></p><code>${esc(j.call)}</code></div>`).join("")}<p class="not">Each one is <code>docs({recipe:"…", args:{…}})</code>; the server fills the call for you (<code>*</code> required, <code>=</code> default). <code>execute({recipe, args, dry_run:true})</code> prices it first.</p></section>
